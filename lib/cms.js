...
var delCode = '<span class="del-col">X</span>',
	...
	defaultHtmlColumn = "<div class='col col-33 html-col movable resizable edit-col'><textarea class='newtextarea'></textarea></div>",
	widgetColumn = '<div class="widget-col col-33 new-col"><select><option value="new-html">Add raw HTML column</option></select><input type="button" class="button add-widget-button" value="Add"></div><div class="clearfix"></div>',
...
$('.add-widget-button').live('click', function () {
    ...
    if ($(this).siblings('select').val() === "new-content-box") {
        $data = '<div class="content-box resizable movable edit-col col col-100"><div class="clearfix"></div><div class="widget-col col-33 new-col"><select><option value="new-html">Add raw HTML column</option>';
        var additionCode = "";
        for (var widget in widgetCode) {
        ...
        $('.newtextarea').redactor();
        $('.newtextarea').removeClass('newtextarea');
    } else {
